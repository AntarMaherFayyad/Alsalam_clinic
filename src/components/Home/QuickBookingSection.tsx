...
export function QuickBookingSection() {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        createAppointment({
            ...
            onError: (err) => console.error('فشل الحجز:', err)
        })
    }
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-14 flex flex-col items-center">
                    ...
                    <p className="text-slate-500 max-w-xl mx-auto leading-relaxed text-sm sm:text-base font-light">
                        املأ النموذج أدناه ببياناتك الصحيحة وسيقوم قسم التنسيق الطبي بالاتصال بك لتأكيد الموعد.
                    </p>
                </div>

                <div className="max-w-2xl mx-auto bg-white rounded-3xl shadow-xl shadow-slate-100 border border-slate-100 p-6 sm:p-10">
                    {submitted ? (
                        <div className="text-center py-10 flex flex-col items-center">
                            ...
                            <p className="text-slate-500 mb-8 max-w-md mx-auto text-sm font-light leading-relaxed">
                                سنتواصل معك هاتفياً في أقرب وقت لتأكيد الموعد.
                            </p>
                            <Button
                                onClick={() => { setSubmitted(false); setForm({ name: '', phone: '', email: '', service: '', date: '', time: '', reason: '' }) }}
                                variant="outline"
                                className="border-slate-200 text-slate-700 bg-white hover:bg-[#0b6a6b] hover:text-white rounded-xl font-bold px-6 h-11 text-sm transition-all"
                            >
                                حجز موعد آخر
                            </Button>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className="h-11 rounded-xl border-slate-200 bg-slate-50/50 focus:border-[#0b6a6b] text-sm"
                                        value={form.name}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        dir="ltr"
                                        className="h-11 rounded-xl border-slate-200 bg-slate-50/50 focus:border-[#0b6a6b] text-sm text-right"
                                        value={form.phone}
                            ...
                            <div className="space-y-2">
                                <Label className="text-xs font-bold text-slate-700 block">الخدمة المطلوبة</Label>
                                <Select
                                    value={form.service}
                                    onValueChange={(v) => setForm({ ...form, service: v || '' })}
                                    disabled={loadingServices}
                                >
                                    <SelectTrigger className="h-11 rounded-xl border-slate-200 bg-slate-50/50 focus:border-[#0b6a6b] text-sm">
                                        <SelectValue placeholder={loadingServices ? 'جاري التحميل...' : 'اختر نوع الخدمة'} />
                                    </SelectTrigger>
                                    <SelectContent className="rounded-xl border-slate-100 shadow-xl">
                                        {services.filter(s => s.visible).map(s => (
                                            <SelectItem key={s.id} value={s.title} className="rounded-lg text-sm py-2.5">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select value={form.time} onValueChange={(v) => setForm({ ...form, time: v || '' })}>
                                        ...
                                        <SelectContent className="rounded-xl border-slate-100 shadow-xl">
                                            {timeSlots.map(t => (
                                                <SelectItem key={t} value={t} className="rounded-lg text-sm py-2.5">{t}</SelectItem>
                                            ))}
                                        </SelectContent>
                                    </Select>
                                </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <Textarea
                                    ...
                                    rows={4}
                                    className="rounded-xl border-slate-200 bg-slate-50/50 focus:border-[#0b6a6b] text-sm resize-none p-4"
                                    value={form.reason}
                            ...
                            <Button
                                ...
                                size="lg"
                                disabled={isPending}
                                className="w-full gap-2 h-12 bg-[#0b6a6b] text-white hover:bg-[#095455] rounded-xl font-bold shadow-md transition-all text-sm"
                            >
                                {isPending ? (
