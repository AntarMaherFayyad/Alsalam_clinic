...
import { Phone, Mail, MapPin, MessageCircle, Send, CheckCircle, Info } from 'lucide-react'
...
export default function ContactPage() {
    ...
    const contactItems = [
        { icon: Phone, title: 'قناة الهاتف المباشر', value: siteSettings.phone, href: `tel:${siteSettings.phone}`, color: 'bg-[#0b6a6b]/10 text-[#0b6a6b]', isLtr: true, label: 'اتصل بنا' },
        { icon: MessageCircle, title: 'المحادثة الفورية', value: 'تواصل عبر واتساب', href: `https://wa.me/${siteSettings.whatsapp}`, color: 'bg-emerald-50 text-emerald-600', isLtr: false, label: 'ابدأ الدردشة' },
        { icon: Mail, title: 'البريد الإلكتروني', value: siteSettings.email, href: `mailto:${siteSettings.email}`, color: 'bg-sky-50 text-sky-600', isLtr: true, label: 'أرسل بريداً' },
        { icon: MapPin, title: 'موقع العيادة', value: siteSettings.address, href: '#map', color: 'bg-amber-50 text-amber-600', isLtr: false, label: 'عرض الموقع' },
    ]

    return (
        <main className="bg-slate-50/30 min-h-screen">
            {/* البانر العلوي المتدرج الاحترافي */}
            <div className="hero-gradient pt-32 pb-16 relative overflow-hidden">
                <div className="absolute inset-0 opacity-10 pointer-events-none">
                    <div className="absolute top-0 right-10 w-72 h-72 rounded-full border border-white -translate-y-1/2" />
                </div>
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
                    ...
                    <p className="text-white/90 max-w-xl mx-auto font-light text-sm sm:text-base">
                        نحن هنا دائماً للإجابة على استفساراتك الطبية والتنظيمية ومساعدتك في الحصول على رعاية متكاملة.
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                    ...
                    <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-10 shadow-xl shadow-slate-100/50">
                        ...
                        <p className="text-xs text-slate-400 mb-6 font-light">املأ الحقول التالية وسيقوم قسم المتابعة بالرد عليك عبر البريد أو الهاتف.</p>

                        {submitted ? (
                            <div className="text-center py-14 flex flex-col items-center">
                                <div className="w-16 h-16 bg-emerald-50 border border-emerald-100 rounded-full flex items-center justify-center mb-5 shadow-sm animate-bounce">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className="h-11 rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:bg-white focus:border-[#0b6a6b] focus:ring-4 focus:ring-[#0b6a6b]/5 text-sm transition-all"
                                            value={form.name}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea
                                        ...
                                        rows={4}
                                        className="rounded-xl border-slate-200 bg-slate-50/50 text-slate-800 focus:bg-white focus:border-[#0b6a6b] focus:ring-4 focus:ring-[#0b6a6b]/5 text-sm transition-all resize-none p-4"
                                        value={form.message}
                                ...
                                <Button
                                    type="submit"
                                    className="w-full gap-2 h-12 bg-[#0b6a6b] text-white hover:bg-[#095455] rounded-xl font-bold shadow-md shadow-[#0b6a6b]/10 transition-all duration-300 text-sm"
                                >
                    ...
                    {/* سكشن الخريطة التفاعلية الفخمة والبيانات */}
                    <div className="space-y-6 h-full flex flex-col">
                        <div className="bg-white border border-slate-100 rounded-3xl p-4 shadow-xl shadow-slate-100/50 flex-1 flex flex-col">
                            ...
                            {/* إطار الخريطة المطور بنقاء بصري عالي وجذاب */}
                            <div id="map" className="relative bg-slate-100 rounded-2xl overflow-hidden min-h-[300px] flex-1 flex items-center justify-center border border-slate-100 mt-4">
                                <div className="text-center p-6 relative z-10">
                                    ...
                                    <p className="text-xs text-slate-500 mt-1 max-w-xs mx-auto leading-relaxed">{siteSettings.address}</p>
                                    <a
                                        href="https://maps.google.com"
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="inline-flex items-center gap-1.5 mt-5 bg-[#0b6a6b] text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-md hover:bg-[#095455] transition-all duration-300"
                                    >
                        ...
                        {/* كارت معلومات إضافية منسق هندسياً */}
                        <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-xl shadow-slate-100/50 space-y-4">
                            ...
                            <div className="flex items-start gap-3.5">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 text-xs font-light leading-relaxed">{siteSettings.address}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-3.5 pt-2">
                                ...
                                <div>
                                    ...
                                    <a href={`tel:${siteSettings.phone}`} className="text-[#0b6a6b] text-xs font-bold font-mono tracking-wider hover:underline" dir="ltr">
                                        {siteSettings.phone}
                                    </a>
                                </div>
                            </div>
