...
export default function ArticlesPage() {
    ...
    const filteredArticles = articles
        ...
        .filter((a) => {
            ...
            const excerptMatch = a.excerpt?.toLowerCase().includes(searchLower);
            const tagsArray = Array.isArray(a.tags) ? a.tags : [];
            const tagsMatch = tagsArray.some((t: string) => t.toLowerCase().includes(searchLower));
            return titleMatch || excerptMatch || tagsMatch;
        });

    if (isLoading) {
        return (
            <div className="max-w-7xl mx-auto px-4 py-32 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 animate-pulse" dir="rtl">
                {[1, 2, 3].map((n) => (
                    <div key={n} className="h-96 bg-slate-100 rounded-2xl" />
                ))}
            </div>
        );
    }
    if (isError) {
        return (
            <div className="text-center py-32 text-rose-500 font-medium text-sm" dir="rtl">
                حدث خطأ أثناء جلب المقالات الطبية. برجاء تحديث الصفحة.
            </div>
        );
    }
    return (
            ...
            <section className="hero-gradient pt-32 pb-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <p className="text-white/80 text-lg max-w-2xl mx-auto leading-relaxed">
                        مقالات طبية متخصصة وموثوقة لرفع الوعي الصحي لضمان سلامتك وسلامة أسرتك بإشراف د. أحمد محمد السالم.
                    </p>
                </div>
            </section>
                ...
                <div className="flex flex-col lg:flex-row gap-6 mb-12 items-stretch lg:items-center justify-between">
                    <div className="relative flex-1 max-w-xl">
                        ...
                        <Input
                            ...
                            className="pr-11 h-11 bg-slate-50 border-slate-200 rounded-xl text-slate-800 placeholder:text-slate-400 focus:bg-white focus:border-[#0b6a6b] focus:ring-4 focus:ring-[#0b6a6b]/5 text-sm transition-all text-right"
                            value={search}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredArticles.map((article) => {
                            ...
                            return (
                                ...
                                >
                                    {/* حاوية الصورة مع حماية الـ Null */}
                                    <div className="relative h-52 overflow-hidden bg-slate-100">
                                        <Image
                                            src={article.image_url || '/images/clinic.png'} // استخدام الحقل والـ fallback الصحيح
                                            alt={article.title}
                                            fill
